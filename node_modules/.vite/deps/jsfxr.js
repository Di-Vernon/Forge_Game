...
var require_riffwave = __commonJS({
  "node_modules/jsfxr/riffwave.js"(exports, module) {
    var FastBase64 = {
    ...
    if (typeof globalThis !== "undefined" && !globalThis.RIFFWAVE) {
      globalThis.RIFFWAVE = RIFFWAVE3;
    }
    if (typeof window !== "undefined" && !window.RIFFWAVE) {
      window.RIFFWAVE = RIFFWAVE3;
    }
  }
});

// node_modules/jsfxr/sfxr.js
var require_sfxr = __commonJS({
  "node_modules/jsfxr/sfxr.js"(exports, module) {
    var SQUARE = 0;
    var SAWTOOTH = 1;
    var SINE = 2;
    var NOISE = 3;
    ...
    function assembleFloat(sign2, exponent, mantissa) {
      return sign2 << 31 | exponent << 23 | mantissa;
    }
    function floatToNumber(flt) {
      ...
      flt = Math.abs(flt);
      if (flt == 0)
        return assembleFloat(sign2, 0, 0);
      var exponent = Math.floor(Math.log(flt) / Math.LN2);
      if (exponent > 127 || exponent < -126)
        return assembleFloat(sign2, 255, 0);
      var mantissa = flt / Math.pow(2, exponent);
      return assembleFloat(sign2, exponent + 127, mantissa * Math.pow(2, 23) & 8388607);
    }
    function numberToFloat(bytes) {
      var sign2 = bytes & 2147483648 ? -1 : 1;
      var exponent = (bytes >> 23 & 255) - 127;
      var significand = bytes & ~(-1 << 23);
      if (exponent == 128)
        return sign2 * (significand ? Number.NaN : Number.POSITIVE_INFINITY);
      if (exponent == -127) {
        if (significand == 0) return sign2 * 0;
        exponent = -126;
        significand /= 1 << 22;
      } else significand = (significand | 1 << 23) / (1 << 23);
      return sign2 * significand * Math.pow(2, exponent);
    }
    ...
    Params.prototype.fromB58 = function(b58encoded) {
      this.fromJSON(sfxr2.b58decode(b58encoded));
      return this;
    };
    ...
    sfxr2 = {};
    sfxr2.toBuffer = function(synthdef) {
      return new SoundEffect(synthdef).getRawBuffer()["buffer"];
    };
    sfxr2.toWebAudio = function(synthdef, audiocontext) {
    ...
    sfxr2.toWave = function(synthdef) {
      return new SoundEffect(synthdef).generate();
    };
    sfxr2.toAudio = function(synthdef) {
      return sfxr2.toWave(synthdef).getAudio();
    };
    sfxr2.play = function(synthdef) {
      return sfxr2.toAudio(synthdef).play();
    };
    sfxr2.b58decode = function(b58encoded) {
    ...
    sfxr2.b58encode = function(synthdef) {
      var p = new Params();
      p.fromJSON(synthdef);
      return p.toB58();
    };
    sfxr2.generate = function(algorithm, options) {
      const p = new Params();
      const opts = options || {};
    ...
    var sliders = {
      ...
      p_base_freq: function(v) {
        return 8 * 44100 * (v * v + 1e-3) / 100;
      },
      p_freq_limit: function(v) {
        return 8 * 44100 * (v * v + 1e-3) / 100;
      },
      p_freq_ramp: function(v) {
        return 1 - Math.pow(v, 3) * 0.01;
      },
      p_freq_dramp: function(v) {
        return -Math.pow(v, 3) * 1e-6;
      },
      p_vib_speed: function(v) {
        return Math.pow(v, 2) * 0.01;
      },
      p_vib_strength: function(v) {
        return v * 0.5;
      },
      p_arp_mod: function(v) {
        return v >= 0 ? 1 - Math.pow(v, 2) * 0.9 : 1 + Math.pow(v, 2) * 10;
      },
      p_arp_speed: function(v) {
        return v === 1 ? 0 : Math.floor(Math.pow(1 - v, 2) * 2e4 + 32);
      },
      p_duty: function(v) {
        return 0.5 - v * 0.5;
      },
      ...
      p_repeat_speed: function(v) {
        return v === 0 ? 0 : Math.floor(Math.pow(1 - v, 2) * 2e4) + 32;
      },
    ...
    var sliders_inverse = {
      ...
      p_base_freq: function(v) {
        return Math.sqrt(v * 100 / 8 / 44100 - 1e-3);
      },
      p_freq_limit: function(v) {
        return Math.sqrt(v * 100 / 8 / 44100 - 1e-3);
      },
      p_freq_ramp: function(v) {
        return Math.cbrt((1 - v) / 0.01);
      },
      p_freq_dramp: function(v) {
        return Math.cbrt(v / -1e-6);
      },
      p_vib_speed: function(v) {
      ...
      p_arp_mod: function(v) {
        return v < 1 ? Math.sqrt((1 - v) / 0.9) : -Math.sqrt((v - 1) / 10);
      },
      p_arp_speed: function(v) {
        return v === 0 ? 1 : 1 - Math.sqrt((v - (v < 100 ? 30 : 32)) / 2e4);
      },
      p_duty: function(v) {
        return (v - 0.5) / -0.5;
      },
      ...
      p_repeat_speed: function(v) {
        return v === 0 ? 0 : -(Math.sqrt((v - 32) / 2e4) - 1);
      },
      p_pha_offset: function(v) {
    ...
    var units = {
      ...
      p_vib_speed: function(v) {
        return v === 0 ? "OFF" : (441e3 / 64 * v).toPrecision(4) + " Hz";
      },
      p_vib_strength: function(v) {
      ...
      p_pha_offset: function(v) {
        return v === 0 ? "OFF" : (1e3 * v / 44100).toPrecision(4) + " msec";
      },
      // Not so sure about this:
      p_pha_ramp: function(v) {
        return v === 0 ? "OFF" : (1e3 * v).toPrecision(4) + " msec/sec";
      },
      p_lpf_freq: function(v) {
      ...
      sound_vol: function(v) {
        ...
        var sign2 = v >= 0 ? "+" : "";
        return sign2 + v.toPrecision(4) + " dB";
      }
    };
    (function(root, factory) {
      var globalRoot = root || (typeof globalThis !== "undefined" ? globalThis : typeof window !== "undefined" ? window : {});
      if (typeof define === "function" && define.amd) {
        define(["./riffwave"], function(RIFFWAVE4) {
          return globalRoot.jsfxr = factory(RIFFWAVE4);
        });
      } else if (typeof module === "object" && module.exports) {
        var RIFFWAVE3 = require_riffwave();
        module.exports = globalRoot.jsfxr = factory(RIFFWAVE3);
      } else {
        globalRoot.jsfxr = factory(globalRoot.RIFFWAVE);
      }
    })(exports, function(RIFFWAVE3) {
      return {
        "sfxr": sfxr2,
        "convert": {
          "sliders": sliders,
...
// node_modules/jsfxr/sfxr.mjs
var import_riffwave = __toESM(require_riffwave(), 1);
var import_sfxr = __toESM(require_sfxr(), 1);
if (typeof globalThis !== "undefined") globalThis.RIFFWAVE = import_riffwave.default;
if (typeof window !== "undefined") window.RIFFWAVE = import_riffwave.default;
var sfxr = import_sfxr.default.sfxr;
var sfxr_default = import_sfxr.default;
var export_jsfxr = import_sfxr.default;
export {
  sfxr_default as default,
  export_jsfxr as jsfxr,
  sfxr
};
//# sourceMappingURL=jsfxr.js.map
